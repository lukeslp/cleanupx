...
```bash
# Basic installation
sudo pip install cleanupx

# With all optional features
sudo pip install cleanupx[all]

# Specific feature sets
sudo pip install cleanupx[ai,documents,images]
```
...
#### AI Features
```bash
sudo pip install cleanupx[ai]
```
**Adds**: OpenAI API integration, LLM-powered analysis

#### Document Processing
```bash
sudo pip install cleanupx[documents]
```
**Adds**: PDF processing, Word document support

#### Enhanced Image Support
```bash
sudo pip install cleanupx[images]
```
**Adds**: HEIC/HEIF support, advanced image formats

#### Archive Support
```bash
sudo pip install cleanupx[archives]
```
**Adds**: RAR archive processing

#### Development Tools
```bash
sudo pip install cleanupx[dev]
```
...
sudo pip install --upgrade cleanupx

# Upgrade with all features
sudo pip install --upgrade cleanupx[all]
```
...
sudo pip install --user cleanupx[all]
...
# Or use trusted host
sudo pip install --trusted-host pypi.org --trusted-host pypi.python.org cleanupx
```
...
sudo pip install --force-reinstall cleanupx[all]
```
...
```bash
# Use user installation
sudo pip3 install --user cleanupx[all]
...
sudo pip install dist/cleanupx-2.0.0-py3-none-any.whl
```
