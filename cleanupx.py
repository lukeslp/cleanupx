...
Features:
- Find and process duplicate files using xAI API
...
- LLM-powered code analysis and consolidation
...
# Import our unified xAI API for legacy support
try:
    ...
    try:
        ...
        XAI_AVAILABLE = False
        logging.warning("xAI unified API not available. Install requirements or check path.")
...
def deduplicate_directory(dir_path: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    logger.info(f"Output directory: {output_dir}")
    logger.info(f"xAI API available: {XAI_AVAILABLE}")
